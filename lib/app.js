...
export class App {
  ...
  actionExport(event) {
    ...
    const exportData = `data:text/json;charset=utf-8,${encodeURIComponent(exportedJson)}`;
    
    const downloadHiddenElem = document.querySelector('#download-hidden');
    downloadHiddenElem.setAttribute('href', exportData);
    downloadHiddenElem.setAttribute('download', 'equalizerContour.json');
    downloadHiddenElem.click();
  }
